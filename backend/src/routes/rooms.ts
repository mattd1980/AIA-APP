...
router.get('/:roomId', requireAuth, async (req, res) => {
  try {
    ...
    const room = await locationService.getRoomById(req.params.roomId, userId);
...
router.post('/:roomId/items', requireAuth, async (req, res) => {
  try {
    ...
    const item = await locationService.createRoomManualItem(req.params.roomId, userId, {
...
router.patch('/:roomId/items/:itemId', requireAuth, async (req, res) => {
  try {
    const userId = (req as AuthenticatedRequest).user!.id;
    const body = req.body as { itemName?: string; category?: string; condition?: string; estimatedValue?: number; replacementValue?: number; notes?: string };
    const item = await locationService.updateRoomItem(req.params.roomId, req.params.itemId, userId, {
      itemName: body.itemName?.trim(),
      category: body.category,
      condition: body.condition,
      estimatedValue: body.estimatedValue,
...
router.delete('/:roomId/items/:itemId', requireAuth, async (req, res) => {
  try {
    const userId = (req as AuthenticatedRequest).user!.id;
    await locationService.deleteRoomItem(req.params.itemId, userId);
...
router.post('/:roomId/analyze', requireAuth, async (req, res) => {
  try {
    const userId = (req as AuthenticatedRequest).user!.id;
    await analysisService.startRoomAnalysis(req.params.roomId, userId);
...
router.patch('/:roomId', requireAuth, async (req, res) => {
  try {
    ...
    const room = await locationService.updateRoom(req.params.roomId, userId, name.trim());
    res.json(room);
  } catch (error: any) {
    if (error.message === 'Room not found') {
      return res.status(404).json({ error: 'Pièce introuvable' });
...
router.delete('/:roomId', requireAuth, async (req, res) => {
  try {
    const userId = (req as AuthenticatedRequest).user!.id;
    await locationService.deleteRoom(req.params.roomId, userId);
...
router.get('/:roomId/images/:imageId', requireAuth, async (req, res) => {
  try {
    const userId = (req as AuthenticatedRequest).user!.id;
    const img = await locationService.getRoomImageById(req.params.imageId, userId);
...
router.delete('/:roomId/images/:imageId', requireAuth, async (req, res) => {
  try {
    const userId = (req as AuthenticatedRequest).user!.id;
    await locationService.deleteRoomImage(req.params.imageId, userId);
